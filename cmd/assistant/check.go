...
func buildSelfCheckReport(cfg config.Config) (string, error) {
	...
	if _, err := os.Stat(configPath); err == nil {
		infoLines = append(infoLines, fmt.Sprintf("config_toml: present path=%s", configPath))
	} else if os.IsNotExist(err) {
		infoLines = append(infoLines, fmt.Sprintf("config_toml: missing path=%s optional=true", configPath))
	} else {
		return "", err
	}
	...
	if mode == "fake" {
		advisories = append(advisories, "fake_client_active: set anthropic.api_key in config.toml to enable real model calls")
	}
	if skillsAdvisory != "" {
	...
	if mode == "fake" {
		lines = append(lines, fmt.Sprintf(
			`next_step: continue with minimal_happy_path step=2 command=%s; set anthropic.api_key in config.toml and rerun %s first if you want real model calls`,
...
func minimalHappyPathLines(mode string) []string {
	...
	if mode == "fake" {
		lines = append(lines, fmt.Sprintf("optional: set anthropic.api_key in config.toml and rerun %s before step 2 if you want real model calls", sourceCommand("check")))
	}
	lines = append(lines,
