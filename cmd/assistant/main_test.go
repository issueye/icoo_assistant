...
func TestPrintUsageIncludesVersionAndExamples(t *testing.T) {
	...
	if !strings.Contains(output, "go run ./icoo_runtime/cmd/assistant --help") {
		t.Fatalf("expected help example, got %q", output)
	}
	if !strings.Contains(output, "go run ./icoo_runtime/cmd/assistant check") {
		t.Fatalf("expected check example, got %q", output)
	}
	if !strings.Contains(output, "go run ./icoo_runtime/cmd/assistant doctor") {
		t.Fatalf("expected doctor alias, got %q", output)
	}
	if !strings.Contains(output, "Run `go run ./icoo_runtime/cmd/assistant check` before the first real task") {
	...
	if !strings.Contains(output, "1. go run ./icoo_runtime/cmd/assistant check") {
		t.Fatalf("expected explicit assistant check step, got %q", output)
	}
	if !strings.Contains(output, "Replace `go run ./icoo_runtime/cmd/assistant` with `assistant` if the binary is already installed.") {
	...
	if !strings.Contains(output, "Without anthropic.api_key in config.toml, assistant runs in fake mode") {
		t.Fatalf("expected mode notes, got %q", output)
	}
	if !strings.Contains(output, "See config.toml.example for supported settings.") {
