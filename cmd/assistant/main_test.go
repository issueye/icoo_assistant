...
func TestPrintUsageIncludesVersionAndExamples(t *testing.T) {
	...
	printUsage(&buf)
	output := buf.String()
	if !strings.Contains(output, "icoo "+Version) {
	...
	if !strings.Contains(output, "Replace `go run ./cmd/assistant` with `icoo` if the binary is already installed.") {
	...
	if !strings.Contains(output, "Without anthropic.api_key in config.toml, icoo runs in fake mode") {
