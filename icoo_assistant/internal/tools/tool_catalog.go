...
func DefaultToolCatalogEntries(includeTask bool) []ToolCatalogEntry {
	entries := []ToolCatalogEntry{
		...
		},
		{
			...
			Description: "Keeps audit queries separate from day-to-day project task operations and supports failure-focused summaries, reason classification, priority failure hints with selection basis, recent context, repeat-pattern hints, sample-target guidance, and recent sample comparison hints, latest samples per reason, recent failure trend hints, plus status- or reason-filtered inspection.",
		},
		{
			Name:        "todo",
...
func renderToolCatalogAuditPaths() string {
	lines := []string{
		...
		`- task_audit action=summary: inspect status counts, failure reason counts, priority failure hints with basis, recent context, repeat-pattern hints, sample-target guidance, and recent sample comparison hints, recent failure trend, and the latest sample for each failure reason before drilling into detailed history or focusing on one failure reason`,
		...
		"recommended_flows:",
		`- task_first: project_task get -> task_audit summary -> inspect priority_failure_basis, priority_failure_context, priority_failure_pattern_hint, priority_failure_sample_target, and priority_failure_sample_compare -> follow priority_failure_hint or task_audit summary reason=<reason> -> task_audit history`,
