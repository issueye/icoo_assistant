...
func DefaultToolCatalogEntries(includeTask bool) []ToolCatalogEntry {
	entries := []ToolCatalogEntry{
		...
		},
		{
			...
			Example:     `{"action":"summary","id":"task-1","reason":"timeout"}`,
			Description: "Keeps audit queries separate from day-to-day project task operations and supports failure-focused summaries, reason classification, latest samples per reason, recent failure trend hints, plus status- or reason-filtered inspection.",
		},
		{
			Name:        "todo",
...
func renderToolCatalogAuditPaths() string {
	lines := []string{
		...
		`- task_audit action=summary: inspect status counts, failure reason counts, recent failure trend, and the latest sample for each failure reason before drilling into detailed history or focusing on one failure reason`,
		...
		"recommended_flows:",
		`- task_first: project_task get -> task_audit summary -> task_audit summary reason=<reason> -> task_audit history`,
