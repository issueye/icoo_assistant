...
func TestToolCatalogList(t *testing.T) {
	...
	if !strings.Contains(result, "- team_message:") {
		t.Fatalf("expected team_message in list, got %q", result)
	}
	if !strings.Contains(result, "- team_protocol:") {
		t.Fatalf("expected team_protocol in list, got %q", result)
	}
...
func TestToolCatalogAuditPaths(t *testing.T) {
	...
	if !strings.Contains(result, "latest_failure_command") {
		t.Fatalf("expected history latest-failure-command guidance, got %q", result)
	}
	if !strings.Contains(result, "latest_failure_error") {
		t.Fatalf("expected history latest-failure-error guidance, got %q", result)
	}
	...
	if !strings.Contains(result, "latest_failure_entry") {
		t.Fatalf("expected history latest-failure-entry guidance, got %q", result)
	}
