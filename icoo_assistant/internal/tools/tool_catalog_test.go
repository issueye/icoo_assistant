...
func TestToolCatalogAuditPaths(t *testing.T) {
	...
	if !strings.Contains(result, "priority failure hints with basis, recent context, repeat-pattern hints, sample-target guidance, recent sample comparison hints, recent failure trend, and lightweight stability-vs-change trend hints") {
