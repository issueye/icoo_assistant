...
func printUsage(out io.Writer) {
	...
	_, _ = fmt.Fprintln(out, "  assistant doctor")
	...
	_, _ = fmt.Fprintln(out, "    1. assistant check")
	_, _ = fmt.Fprintln(out, `    2. assistant "先用 tool_catalog 总结当前可用工具，再说明 project_task、task_audit 和 agent_hook_audit 的边界"`)
	_, _ = fmt.Fprintln(out, `    3. assistant "创建一个项目任务，用于验证后台测试"`)
	_, _ = fmt.Fprintln(out, `    4. assistant "使用 tool_catalog action=audit_paths 说明审计入口，再给出 task_audit 和 agent_hook_audit 的查询示例"`)
	_, _ = fmt.Fprintln(out)
