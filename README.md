...
go run ./cmd/assistant "先用 task_audit action=summary 看 priority_failure_reason、priority_failure_basis、priority_failure_context、priority_failure_pattern_hint、priority_failure_sample_target、priority_failure_sample_compare 和 priority_failure_hint，再决定先排查哪类失败"
...
当前已经为 Agent 主循环补上基础 hook 埋点，默认会把事件写入工作区的 `.agent-hooks/events.jsonl`。`0.1.14` 继续把异常复盘路径往前推了一步，任务侧现在不仅能定位优先失败样本，还能快速对比最近两条同类失败样本的变化点。当前埋点覆盖了：
...
为了让演示和上手路径更顺滑，`0.1.14` 继续把 `tool_catalog` 作为统一工具说明入口，并补上了更适合异常复盘的失败样本对比提示说明。当前推荐的职责边界可以简单记成：
...
- 下一轮 `v0.1.15` 版本计划见 [docs/v0.1.15-开发计划.md](E:\codes\icoo_assistant\docs\v0.1.15-开发计划.md)
