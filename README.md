...
当前仓库已经在 `0.1.30` 初代可用版基线上完成若干轮稳定化迭代，当前主线聚焦于单 Agent 本地执行、任务持久化、审计与后台运行能力。此前内置的 `team` 原型能力已经下线，后续会通过独立管理服务重新承载。
...
- `ANTHROPIC_BASE_URL`：可选，自定义 Anthropic 兼容网关或代理地址
...
当前版本已经移除了内置 `team` 功能，后续如果恢复多人协作能力，将通过独立管理服务承载，而不是继续在本地进程内维护 `.team/` 目录和 `team_*` 工具。
