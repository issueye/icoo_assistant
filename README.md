...
当前已经为 Agent 主循环补上基础 hook 埋点，默认会把事件写入工作区的 `.agent-hooks/events.jsonl`。`0.1.8` 继续把异常复盘路径往前推了一步，任务侧现在不仅能看失败趋势，还能直接按失败原因聚焦。当前埋点覆盖了：
...
为了让演示和上手路径更顺滑，`0.1.8` 继续把 `tool_catalog` 作为统一工具说明入口，并补上了更适合异常复盘的失败原因聚焦用法。当前推荐的职责边界可以简单记成：
...
- 下一轮 `v0.1.9` 版本计划见 [docs/v0.1.9-开发计划.md](E:\codes\icoo_assistant\docs\v0.1.9-开发计划.md)
