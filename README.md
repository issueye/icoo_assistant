...
当前仓库已经在 `0.1.30` 初代可用版基线上完成 `0.1.31` 与 `0.1.32` 两轮 post-release hardening，并在 `0.1.33-v0.1.35` 正式切入 Team 基础设施主线，能力范围包括：
...
`0.1.35` 的定位仍然不是“功能很多”，而是“在保持最小闭环清晰的前提下，把 Team 的最小通信链路先落到磁盘与工具层”。当前版本更适合：
...
- 当前只到了 Team config、teammate registry、inbox message store 和最小 request/response thread，还没有真实 teammate loop、协议审批和自治认领
